...
from nbd_server.protocol.messages import Requests, Responses, receive_exactly


class TestReceiveExactly(unittest.IsolatedAsyncioTestCase):

    async def test_receive_exactly_single_chunk(self):
        test_data = b"hello"
    ...
    async def test_receive_exactly_empty(self):
        test_data = b""
        reader = asyncio.StreamReader()
        reader.feed_data(test_data)
        reader.feed_eof()
    ...
    async def test_receive_exactly_large_data(self):
    ...
    async def test_receive_exactly_multiple_chunks(self):
        reader = asyncio.StreamReader()
        reader.feed_data(b"hello")
        reader.feed_data(b"world")
        reader.feed_eof()

        result = await receive_exactly(reader, 10)

        self.assertEqual(result, b"helloworld")

    async def test_receive_exactly_insufficient_data(self):
        reader = asyncio.StreamReader()
        reader.feed_data(b"hello")
        reader.feed_eof()

        with self.assertRaises(asyncio.IncompleteReadError):
            await receive_exactly(reader, 10)
...
class TestResponses(unittest.TestCase):
    ...
    def test_simple_reply_with_error(self):
        error_code = 5
        handle = 67890
    ...
    def test_info_reply_large_export(self):
        option = 7
        export_size = 10995116277760
    ...
    def test_ack_reply_different_options(self):
        for option in [1, 3, 7, 8]:
            result = Responses.ack_reply(option)
...
class TestRequests(unittest.TestCase):
    ...
    def test_client_flags_multiple_bits(self):
        flags_value = 0x00000003
        data = struct.pack(">I", flags_value)
    ...
    def test_option_request_empty_data(self):
        option = 1
        length = 0
        option_data = b""
        header = struct.pack(">QII", IHAVEOPT, option, length)
    ...
    def test_command_write_operation(self):
        flags = 0
        cmd_type = 1
        handle = 99999
        offset = 2048
